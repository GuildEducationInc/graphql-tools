...
import { stitchingDirectives } from '../../stitching-directives/src';

describe('directive merging using type merging', () => {
  ...
  describe('guild schema', () => {
    ...
    let enrollmentSchema = makeExecutableSchema({
      typeDefs: /* GraphQL */ `
        ...
        type Query {
          student(id: String!): Student @merge(keyField: "id")
          studentsWithEnrollments(ids: [String!]!): [Student!]! @merge(keyField: "id", keyArg: "ids")
        }
    ...
    let majorsSchema = makeExecutableSchema({
      typeDefs: /* GraphQL */ `
        ...
        type Query {
          ...
          majorsByIds(ids: [String!]!): [Major!]! @merge(keyField: "id", keyArg: "ids")
        }
